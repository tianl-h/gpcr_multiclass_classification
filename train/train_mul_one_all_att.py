...
# @Last Modified time: 2021-03-04 02:32:34
...
def main():
    ...
    for method, func_and_hyper in METHODS_WITH_HYPER.items():
        ...
        for train, test in cv.split(X_train_prime, y_train_prime):
            ...
            roc_auc, F1_score, ACC, Cohan_Kappa, MCC, precision, recall = CalculateMetrics(y_train_prime[test], y_val_pred, y_val_pred_prime, method)
            if not att in metrics:
                metrics[att] = {}
                if not method in metrics[att]:
                    metrics[att][method] = []
            metrics[att][method] = ["%.3f" % roc_auc, "%.3f" % F1_score,"%.3f" % ACC,"%.3f" % Cohan_Kappa, "%.3f" % MCC, "%.3f" % precision,"%.3f" % recall]
        ...
        for att, m_dict in metrics.items():
            file = open(os.path.join('results_all_att/Validation_Radar/', att + "_metrics_test.txt"), "w")
    ...
    #     autosize=True,
        ...
        for att, m_dict in metrics.items():
            for method in m_dict.keys():
                ...
                F1_score = float(metrics[att][method][1])
                ACC = float(metrics[att][method][2])
                Cohan_Kappa = float(metrics[att][method][3])
                MCC = float(metrics[att][method][4])
                precision = float(metrics[att][method][5])
                recall = float(metrics[att][method][6])

                radar_chart.add_trace(
                    go.Scatterpolar(
                        name = method,
                        r=[roc_auc, F1_score, ACC, Cohan_Kappa, MCC, precision, recall, roc_auc],
                        theta=['AUC', 'F1 Score', 'ACC', 'Cohan Kappa', 'MCC', 'Precision', 'Recall', 'AUC'],
                        showlegend=True
                        )
                )
            ...
            radar_chart.write_image("results_all_att/Validation_Radar/" + att + ".png")
            radar_chart.data = []
